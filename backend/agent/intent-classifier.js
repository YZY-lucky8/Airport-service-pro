...
class IntentClassifier {
  constructor() {
    ...
    this.intentLibrary = {
      ...
      // 3. 航班查询
      flight_query: {
        keywords: ['航班', '航班号', '起飞', '到达', '延误', '取消', '取消了吗', '什么时候飞', '几点起飞', 'flight', '帮我查', '查一下'],
        regex: [/航班.*状态/, /.*延误/, /.*取消/, /.*起飞.*时间/, /.*几点.*飞/, /航班.*号/, /查.*[A-Z]{1,4}\d{3,4}/, /\b[A-Z]{1,4}\d{3,4}\b/],
        entities: ['flight_no'],
        priority: 85,
      },
      ...
      // 4. 知识库问答
      knowledge_qa: {
        keywords: ['充电宝', '托运', '能带', '可以带', '规定', '规则', '液体', '刀具', '打火机', '婴儿', '儿童票', '停车', 'wifi', '停车费', '卫生间', '厕所', '孕妇', '怀孕', '老人', '轮椅', '宠物', '小猫', '小狗', '导盲犬', '残疾人', '残障', '儿童', '出国', '国际', '护照', '签证', '休息室', '充电', '老人机', '放大字体'],
        regex: [/.*能.*带.*/, /.*可以.*带.*/, /.*能.*托运.*/, /.*规定.*/, /充电宝/, /打火机/, /液体.*带/, /停车.*费/, /.*孕妇.*/, /.*怀孕.*/, /.*老人.*飞机.*/, /.*宠物.*/, /.*带.*猫.*/, /.*带.*狗.*/, /.*国际.*/, /.*出国.*/, /.*老人机.*/, /.*放大.*字体.*/],
        entities: ['topic'],
        priority: 80,
      },
