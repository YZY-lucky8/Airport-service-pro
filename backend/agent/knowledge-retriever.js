...
class KnowledgeRetriever {
  ...
  async _keywordMatch(query, category, limit) {
    ...
    // 中文无空格分词：滑动窗口拆2-4字符子串
    const tokens = new Set();
    for (let len = 2; len <= Math.min(5, query.length); len++) {
    ...
    const tokenArr = Array.from(tokens).slice(0, 10); // 最多取10个token避免SQL过大
    ...
    // 任一token命中 keywords、title 或 content 即算匹配
    const orParts = tokenArr
      .map(
